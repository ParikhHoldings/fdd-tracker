...
class BuyerReportDeliveryQueueIn(BaseModel):
    email: EmailStr
    left_slug: str
    right_slug: str
    channel: str = "telegram"
    template_variant: str = "executive"
    days: int = 7
    max_alerts: int = 200
    comparison_limit: int = 200
    include_health_signals: bool = False
...
@app.get("/health-signals/{franchise_slug}/summary/markdown")
def health_signals_summary_markdown(
    franchise_slug: str,
    source: str | None = Query(default=None),
    limit: int = Query(default=200, ge=1, le=500),
) -> dict:
    payload = get_health_signal_summary(franchise_slug=franchise_slug, source=source, limit=limit)
    return {"franchise_slug": franchise_slug, "source": source, "markdown": render_health_signal_summary_markdown(payload)}


@app.get("/health-signals/{franchise_slug}/summary/telegram")
def health_signals_summary_telegram(
    franchise_slug: str,
    source: str | None = Query(default=None),
    limit: int = Query(default=200, ge=1, le=500),
    ...
    payload = get_health_signal_summary(franchise_slug=franchise_slug, source=source, limit=limit)
    chunks = render_health_signal_summary_telegram_chunks(payload, max_chars=max_chars)
    return {
        "franchise_slug": franchise_slug,
        "source": source,
        "max_chars": max_chars,
...
@app.get("/health-signals/{franchise_slug}/summary/csv")
def health_signals_summary_csv(
    franchise_slug: str,
    source: str | None = Query(default=None),
    limit: int = Query(default=200, ge=1, le=500),
) -> dict:
    payload = get_health_signal_summary(franchise_slug=franchise_slug, source=source, limit=limit)
    return {"franchise_slug": franchise_slug, "source": source, "csv": render_health_signal_summary_csv(payload)}


@app.get("/health-signals/{franchise_slug}/summary/packet")
def health_signals_summary_packet(
    franchise_slug: str,
    source: str | None = Query(default=None),
    limit: int = Query(default=200, ge=1, le=500),
    max_chars: int = Query(default=2500, ge=200, le=10000),
    ...
    payload = get_health_signal_summary(franchise_slug=franchise_slug, source=source, limit=limit)
    packet = build_health_signal_summary_packet(payload, max_chars=max_chars)
    return {"franchise_slug": franchise_slug, "source": source, **packet}
...
def _build_buyer_report_delivery_envelope(
    ...
    return {
        ...
        "delivery_metadata": {
            "dispatch_ready": True,
            "dispatch_policy": get_outbox_row_dispatch_policy({"kind": "buyer-report-envelope", "channel": channel, "email": payload["email"]}),
...
def buyer_report_comparison_brief_delivery_envelope_options() -> dict:
    ...
    return {
        ...
        "surfaces": {
            ...
            "delivery_envelope": "/buyer-reports/comparison-brief/delivery-envelope",
            "queue": "/buyer-reports/comparison-brief/delivery-envelope/queue",
        },
    }
...
@app.post("/buyer-reports/comparison-brief/delivery-envelope/queue")
def buyer_report_comparison_brief_delivery_envelope_queue(payload: BuyerReportDeliveryQueueIn) -> dict:
    channel = payload.channel.strip().lower()
    template_variant = payload.template_variant.strip().lower()

    if channel not in {"email", "telegram", "slack"}:
        return {"queued": False, "error": "unsupported_channel", "supported": ["email", "telegram", "slack"]}
    if template_variant not in {"executive", "analyst", "concise"}:
        return {"queued": False, "error": "unsupported_template_variant", "supported": ["executive", "analyst", "concise"]}

    bundle = buyer_report_comparison_brief(
        email=payload.email,
        left_slug=payload.left_slug,
        right_slug=payload.right_slug,
        days=max(1, min(payload.days, 30)),
        max_alerts=max(1, min(payload.max_alerts, 1000)),
        comparison_limit=max(1, min(payload.comparison_limit, 500)),
        include_health_signals=payload.include_health_signals,
        health_limit=max(1, min(payload.health_limit, 500)),
        max_chars=max(200, min(payload.max_chars, 10000)),
    )
    envelope = _build_buyer_report_delivery_envelope(
        payload=bundle,
        template_variant=template_variant,
    ...
    run_id = payload.run_id or f"buyer-report-{int(datetime.now(timezone.utc).timestamp())}"
    outbox_row = {
        ...
        "run_id": run_id,
        "metadata": envelope["delivery_metadata"],
    }
    outbox_row["dispatch_policy"] = get_outbox_row_dispatch_policy(outbox_row)
    outbox_path = write_outbox_row(row=outbox_row)
