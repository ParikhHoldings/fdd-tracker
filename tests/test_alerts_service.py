...
from fdd_tracker.services.alerts import build_provider_catalog_options_packet, build_provider_health_options_packet, render_provider_catalog_options_csv, render_provider_catalog_options_markdown, render_provider_catalog_options_telegram_chunks, render_provider_health_options_csv, render_provider_health_options_markdown, render_provider_health_options_telegram_chunks
...
def test_summarize_provider_health_recommendations():
    data = summarize_provider_health_recommendations(["noop", "resend", "not-real"])
...
def test_render_provider_health_recommendations_markdown_and_telegram():
    markdown = render_provider_health_recommendations_markdown(["noop", "resend", "not-real"])
    assert "# Provider Health Recommendations" in markdown
    telegram = render_provider_health_recommendations_telegram_chunks(["noop", "resend", "not-real"], max_chars=220)
    assert telegram["chunk_count"] >= 1
    assert telegram["chunks_with_index"][0].startswith("[1/")


def test_render_provider_health_recommendations_csv():
    csv_text = render_provider_health_recommendations_csv(["noop", "resend", "not-real"])
    assert "metric,value" in csv_text
    assert "severity,code,message,action,env_key,count,providers,supported" in csv_text


def test_build_provider_health_recommendations_packet():
    payload = build_provider_health_recommendations_packet(["noop", "resend", "not-real"], max_chars=220)
    assert "recommendations" in payload
    assert "markdown" in payload
    assert "csv" in payload
    assert "telegram" in payload
    assert payload["telegram"]["chunk_count"] >= 1


def test_render_provider_health_summary_markdown():
    md = render_provider_health_summary_markdown(["noop", "resend", "not-real"])
    assert "# Provider Health Summary" in md
    assert "## Missing env frequency" in md
    assert "noop" in md


def test_render_provider_health_summary_telegram_chunks():
    payload = render_provider_health_summary_telegram_chunks(["noop", "resend", "not-real"], max_chars=220)
    assert payload["chunk_count"] >= 1
    assert len(payload["chunks"]) == payload["chunk_count"]
    assert payload["chunks_with_index"][0].startswith("[1/")


def test_render_provider_health_summary_csv():
    csv_text = render_provider_health_summary_csv(["noop", "resend", "not-real"])
    assert "metric,value" in csv_text
    assert "provider,known,ready,supports_live,missing_env" in csv_text
    assert "counts.total" in csv_text


def test_build_provider_health_summary_packet():
    payload = build_provider_health_summary_packet(["noop", "resend", "not-real"], max_chars=220)
...
def test_get_provider_health_details_supported_provider():
    data = get_provider_health_details("noop")
    assert data["provider"] == "noop"
    assert data["supported"] is True
    assert data["health"]["known"] is True


def test_get_provider_health_details_unknown_provider():
    data = get_provider_health_details("not-real")
...
def test_provider_recommendations_renderers_and_packet():
    markdown = render_provider_recommendations_markdown("noop")
    assert "# Provider Recommendations" in markdown
    telegram = render_provider_recommendations_telegram_chunks("noop", max_chars=220)
    assert telegram["chunk_count"] >= 1
    assert telegram["chunks_with_index"][0].startswith("[1/")
    csv_text = render_provider_recommendations_csv("noop")
    assert "metric,value" in csv_text
    assert "priority,code,message,reason,missing_env,supported_providers,endpoint" in csv_text
    packet = build_provider_recommendations_packet("noop", max_chars=220)
...
def test_provider_details_options_renderers_and_packet():
    markdown = render_provider_details_options_markdown()
    assert "# Provider Details Options" in markdown
    assert "## Surfaces" in markdown
    assert "/alerts/providers/details/options/packet" in markdown

    tg = render_provider_details_options_telegram_chunks(max_chars=200)
    assert tg["chunk_count"] >= 1
    assert tg["chunks_with_index"][0].startswith("[1/")

    csv_text = render_provider_details_options_csv()
    assert "section,key,value" in csv_text
    assert "surfaces,options,/alerts/providers/details/options" in csv_text

    packet = build_provider_details_options_packet(max_chars=200)
    assert "options" in packet
    assert "markdown" in packet
    assert "csv" in packet
    assert "telegram" in packet
    assert packet["telegram"]["chunk_count"] >= 1


def test_get_provider_health_options_contract():
    data = get_provider_health_options()
    ...
    assert data["surfaces"]["health_details"] == "/alerts/providers/{provider}/health"
    assert data["surfaces"]["health_summary_markdown"] == "/alerts/providers/health/summary/markdown"
    assert data["surfaces"]["health_summary_csv"] == "/alerts/providers/health/summary/csv"
    assert data["surfaces"]["health_summary_packet"] == "/alerts/providers/health/summary/packet"
    assert data["surfaces"]["health_summary_recommendations_csv"] == "/alerts/providers/health/summary/recommendations/csv"


def test_provider_health_options_renderers_and_packet():
    markdown = render_provider_health_options_markdown()
    assert "# Provider Health Options" in markdown
    assert "/alerts/providers/health/options/packet" in markdown

    tg = render_provider_health_options_telegram_chunks(max_chars=200)
    assert tg["chunk_count"] >= 1
    assert tg["chunks_with_index"][0].startswith("[1/")

    csv_text = render_provider_health_options_csv()
    assert "section,key,value" in csv_text
    assert "surfaces,options,/alerts/providers/health/options" in csv_text

    packet = build_provider_health_options_packet(max_chars=200)
    assert "options" in packet
    assert "markdown" in packet
    assert "csv" in packet
    assert "telegram" in packet
...
def test_get_provider_health_summary_options_contract():
    data = get_provider_health_summary_options()
    assert data["constraints"]["provider"]["type"] == "csv|string|null"
    assert data["constraints"]["max_chars"]["minimum"] == 100
    assert data["defaults"]["max_chars"] == 3500
    ...
    assert data["surfaces"]["recommendations"] == "/alerts/providers/health/summary/recommendations"
    assert data["surfaces"]["recommendations_csv"] == "/alerts/providers/health/summary/recommendations/csv"
    assert data["surfaces"]["recommendations_packet"] == "/alerts/providers/health/summary/recommendations/packet"


def test_provider_health_summary_options_renderers_and_packet():
    markdown = render_provider_health_summary_options_markdown()
    assert "# Provider Health Summary Options" in markdown
    assert "## Surfaces" in markdown
    assert "/alerts/providers/health/summary/options/packet" in markdown

    tg = render_provider_health_summary_options_telegram_chunks(max_chars=200)
    assert tg["chunk_count"] >= 1
    assert tg["chunks_with_index"][0].startswith("[1/")

    csv_text = render_provider_health_summary_options_csv()
    assert "section,key,value" in csv_text
    assert "surfaces,options,/alerts/providers/health/summary/options" in csv_text

    packet = build_provider_health_summary_options_packet(max_chars=200)
    assert "options" in packet
    assert "markdown" in packet
    assert "csv" in packet
    assert "telegram" in packet
...
def test_provider_recommendations_options_renderers_and_packet():
    markdown = render_provider_recommendations_options_markdown()
    assert "# Provider Recommendations Options" in markdown
    assert "## Surfaces" in markdown
    assert "/alerts/providers/recommendations/options/packet" in markdown

    tg = render_provider_recommendations_options_telegram_chunks(max_chars=200)
    assert tg["chunk_count"] >= 1
    assert tg["chunks_with_index"][0].startswith("[1/")

    csv_text = render_provider_recommendations_options_csv()
    assert "section,key,value" in csv_text
    assert "surfaces,options,/alerts/providers/recommendations/options" in csv_text

    packet = build_provider_recommendations_options_packet(max_chars=200)
    assert "options" in packet
    assert "markdown" in packet
    assert "csv" in packet
    assert "telegram" in packet
