...
def render_weekly_brief_options_markdown() -> str:
    ...
    for key in sorted(constraints.keys()):
        lines.append(f"- {key}: {json.dumps(constraints[key], sort_keys=True)}")
...
def render_weekly_brief_options_telegram_chunks(max_chars: int = 3500) -> dict:
    markdown = render_weekly_brief_options_markdown()
    max_chars = max(100, min(max_chars, 4096))
...
def render_weekly_brief_options_csv() -> str:
    payload = get_weekly_brief_options()
    out = StringIO()
    writer = csv.writer(out)
    writer.writerow(["section", "key", "value"])
...
def build_weekly_brief_options_packet(max_chars: int = 3500) -> dict:
    return {
        "options": get_weekly_brief_options(),
        "markdown": render_weekly_brief_options_markdown(),
        "csv": render_weekly_brief_options_csv(),
        "telegram": render_weekly_brief_options_telegram_chunks(max_chars=max_chars),
    }
    ...
    return {
        "constraints": {
            ...
            "max_chars": {
                ...
                "maximum": 4096,
                "applies_to": [
                    "/alerts/providers/health/options/telegram",
                    "/alerts/providers/health/options/packet",
                ],
            },
        },
        "defaults": {
        ...
        "surfaces": {
            ...
            "options_markdown": "/alerts/providers/health/options/markdown",
            "options_telegram": "/alerts/providers/health/options/telegram",
            "options_csv": "/alerts/providers/health/options/csv",
            "options_packet": "/alerts/providers/health/options/packet",
            "health": "/alerts/providers/health",
            "health_summary": "/alerts/providers/health/summary",
...
def render_provider_health_options_markdown() -> str:
    payload = get_provider_health_options()
    providers = payload.get("providers", {})
    constraints = payload.get("constraints", {})
    surfaces = payload.get("surfaces", {})
...
def render_provider_health_options_telegram_chunks(max_chars: int = 3500) -> dict:
...
def render_provider_health_options_csv() -> str:
    payload = get_provider_health_options()
    out = StringIO()
    writer = csv.writer(out)
    writer.writerow(["section", "key", "value"])
...
def build_provider_health_options_packet(max_chars: int = 3500) -> dict:
    return {
        "options": get_provider_health_options(),
        "markdown": render_provider_health_options_markdown(),
        "csv": render_provider_health_options_csv(),
        "telegram": render_provider_health_options_telegram_chunks(max_chars=max_chars),
    }
...
def get_provider_health_details(provider: str) -> dict:
    provider_name = (provider or "").strip().lower()
    health = get_provider_health(provider_name)
    return {
        "provider": provider_name,
        "supported": bool(health.get("known")),
        "health": health,
    }
...
def get_provider_details_options() -> dict:
    ...
    return {
        ...
        "surfaces": {
            ...
            "options_markdown": "/alerts/providers/details/options/markdown",
            "options_telegram": "/alerts/providers/details/options/telegram",
            "options_csv": "/alerts/providers/details/options/csv",
            "options_packet": "/alerts/providers/details/options/packet",
            "details": "/alerts/providers/{provider}",
            "recommendations": "/alerts/providers/{provider}/recommendations",
            "recommendations_options": "/alerts/providers/recommendations/options",
            "health": "/alerts/providers/health",
...
def render_provider_details_options_markdown() -> str:
    payload = get_provider_details_options()
    providers = payload.get("providers", {})
    constraints = payload.get("constraints", {})
    surfaces = payload.get("surfaces", {})
    ...
    lines = [
        ...
        "",
        "## Constraints",
        f"- provider.examples: {', '.join(constraints.get('provider', {}).get('examples', [])) or '(none)'}",
        "",
        "## Surfaces",
    ]
    for key in sorted(surfaces.keys()):
...
def render_provider_details_options_telegram_chunks(max_chars: int = 3500) -> dict:
...
def render_provider_details_options_csv() -> str:
    payload = get_provider_details_options()
    out = StringIO()
    writer = csv.writer(out)
    writer.writerow(["section", "key", "value"])
...
def build_provider_details_options_packet(max_chars: int = 3500) -> dict:
    return {
        "options": get_provider_details_options(),
        "markdown": render_provider_details_options_markdown(),
        "csv": render_provider_details_options_csv(),
        "telegram": render_provider_details_options_telegram_chunks(max_chars=max_chars),
    }
...
def get_provider_recommendations_options() -> dict:
    ...
    return {
        ...
        "surfaces": {
            ...
            "options_markdown": "/alerts/providers/recommendations/options/markdown",
            "options_telegram": "/alerts/providers/recommendations/options/telegram",
            "options_csv": "/alerts/providers/recommendations/options/csv",
            "options_packet": "/alerts/providers/recommendations/options/packet",
            "recommendations": "/alerts/providers/{provider}/recommendations",
...
def render_provider_recommendations_options_markdown() -> str:
    payload = get_provider_recommendations_options()
    providers = payload.get("providers", {})
    constraints = payload.get("constraints", {})
    surfaces = payload.get("surfaces", {})
    ...
    lines = [
        ...
        f"- supported: {', '.join(providers.get('supported', [])) or '(none)'}",
        f"- live_capable: {', '.join(providers.get('live_capable', [])) or '(none)'}",
        f"- ready: {', '.join(providers.get('ready', [])) or '(none)'}",
        "",
        "## Constraints",
        f"- provider.examples: {', '.join(constraints.get('provider', {}).get('examples', [])) or '(none)'}",
        f"- max_chars: {constraints.get('max_chars', {}).get('minimum', '')}..{constraints.get('max_chars', {}).get('maximum', '')}",
...
def render_provider_recommendations_options_telegram_chunks(max_chars: int = 3500) -> dict:
...
def render_provider_recommendations_options_csv() -> str:
    payload = get_provider_recommendations_options()
    out = StringIO()
    writer = csv.writer(out)
    writer.writerow(["section", "key", "value"])
...
def build_provider_recommendations_options_packet(max_chars: int = 3500) -> dict:
    return {
        "options": get_provider_recommendations_options(),
        "markdown": render_provider_recommendations_options_markdown(),
        "csv": render_provider_recommendations_options_csv(),
        "telegram": render_provider_recommendations_options_telegram_chunks(max_chars=max_chars),
    }
